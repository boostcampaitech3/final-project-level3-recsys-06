...
if st.session_state.NFT_name == 'Otherdeed for Otherside':
    ...
    config = load_config(config_path="/opt/ml/Final/dev_config.yaml")
    ...
    # conn1 = pymysql.connect(host=host1, user=user1, password=password1, db=db_name1, port=port1, charset='utf8', autocommit=True, cursorclass=pymysql.cursors.DictCursor)
    ...
    if 'data_dataframe' not in st.session_state:
        st.session_state.data_dataframe=pd.read_csv('/opt/ml/Final/gcp_ITEM_dataframe.csv')

    # TRADE
    if 'data_dataframe1' not in st.session_state:
        st.session_state.data_dataframe1=pd.read_csv('/opt/ml/Final/gcp_TRADE_dataframe.csv')

    # PRICE_OFFER
    if 'data_dataframe' not in st.session_state:
        st.session_state.data_dataframe2=pd.read_csv('/opt/ml/Final/gcp_PRICE_OFFER_dataframe.csv')

    if 'sim_dataframe' not in st.session_state:
        st.session_state.sim_dataframe=pd.read_csv('/opt/ml/Final/gcp_similarity_ITEM.csv')
    ...
    if (st.session_state.token_num>=0) and (st.session_state.token_num<=9):
        ...
        # 유사한 아이템 보여주기
        Similarity=(st.session_state.sim_dataframe.values[st.session_state.idx])@(st.session_state.sim_dataframe.T)
        Similarity[st.session_state.idx]=-np.inf
        Similarity_10=sorted(np.argsort(-(Similarity))[:10])
        
        st.subheader('해당 상품과 비슷한 상품입니다!!!')
        col0, col1, col2, col3, col4= st.columns(5)
        with col0:
            st.session_state.sim_button0=st.button(label=f'#{st.session_state.data_dataframe.loc[Similarity_10[0]][1]}',on_click=change_idx,args=(Similarity_10[0],))
            st.image(st.session_state.data_dataframe.loc[Similarity_10[0]][6])
            st.caption(f"가격 : {token_price[0]} ETH")
            
        with col1:
            # st.subheader(f'#{data_dataframe.loc[1][1]}')
            st.session_state.sim_button1=st.button(label=f'#{st.session_state.data_dataframe.loc[Similarity_10[1]][1]}',on_click=change_idx,args=(Similarity_10[1],))
            st.image(st.session_state.data_dataframe.loc[Similarity_10[1]][6])
            st.caption(f"가격 : {token_price[1]} ETH")

        with col2:
            # st.subheader(f'#{data_dataframe.loc[2][1]}')
            st.session_state.sim_button2=st.button(label=f'#{st.session_state.data_dataframe.loc[Similarity_10[2]][1]}',on_click=change_idx,args=(Similarity_10[2],))
            st.image(st.session_state.data_dataframe.loc[Similarity_10[2]][6])
            st.caption(f"가격 : {token_price[2]} ETH")

        with col3:
            # st.subheader(f'#{data_dataframe.loc[3][1]}')
            st.session_state.sim_button3=st.button(label=f'#{st.session_state.data_dataframe.loc[Similarity_10[3]][1]}',on_click=change_idx,args=(Similarity_10[3],))
            st.image(st.session_state.data_dataframe.loc[Similarity_10[3]][6])
            st.caption(f"가격 : {token_price[3]} ETH")

        with col4:
            # st.subheader(f'#{data_dataframe.loc[4][1]}')
            st.session_state.sim_button4=st.button(label=f'#{st.session_state.data_dataframe.loc[Similarity_10[4]][1]}',on_click=change_idx,args=(Similarity_10[4],))
            st.image(st.session_state.data_dataframe.loc[Similarity_10[4]][6])
            st.caption(f"가격 : {token_price[4]} ETH")

        col5, col6, col7, col8, col9= st.columns(5)
        with col5:
            # st.subheader(f'#{data_dataframe.loc[5][1]}')
            st.session_state.sim_button5=st.button(label=f'#{st.session_state.data_dataframe.loc[Similarity_10[5]][1]}',on_click=change_idx,args=(Similarity_10[5],))
            st.image(st.session_state.data_dataframe.loc[Similarity_10[5]][6])
            st.caption(f"가격 : {token_price[5]} ETH")

        with col6:
            # st.subheader(f'#{data_dataframe.loc[6][1]}')
            st.session_state.sim_button6=st.button(label=f'#{st.session_state.data_dataframe.loc[Similarity_10[6]][1]}',on_click=change_idx,args=(Similarity_10[6],))
            st.image(st.session_state.data_dataframe.loc[Similarity_10[6]][6])
            st.caption(f"가격 : {token_price[6]} ETH")

        with col7:
            # st.subheader(f'#{data_dataframe.loc[7][1]}')
            st.session_state.sim_button7=st.button(label=f'#{st.session_state.data_dataframe.loc[Similarity_10[7]][1]}',on_click=change_idx,args=(Similarity_10[7],))
            st.image(st.session_state.data_dataframe.loc[Similarity_10[7]][6])
            st.caption(f"가격 : {token_price[7]} ETH")

        with col8:
            # st.subheader(f'#{data_dataframe.loc[8][1]}')
            st.session_state.sim_button8=st.button(label=f'#{st.session_state.data_dataframe.loc[Similarity_10[8]][1]}',on_click=change_idx,args=(Similarity_10[8],))
            st.image(st.session_state.data_dataframe.loc[Similarity_10[8]][6])
            st.caption(f"가격 : {token_price[8]} ETH")

        with col9:
            # st.subheader(f'#{data_dataframe.loc[9][1]}')
            st.session_state.sim_button9=st.button(label=f'#{st.session_state.data_dataframe.loc[Similarity_10[9]][1]}',on_click=change_idx,args=(Similarity_10[9],))
            st.image(st.session_state.data_dataframe.loc[Similarity_10[9]][6])
